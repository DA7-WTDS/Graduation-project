...
/// </summary>
/// <remarks>
/// Date-parameterised: the nightly tick runs for today, while the § C fidelity lane
/// walks it across history one session at a time. Everything that used to read the
/// wall clock now reads <c>runDate</c>, or a replayed 2025 portfolio would be dated
/// today, judged against today's rebalance clock, and priced from today's run.
/// </remarks>
internal sealed class ShadowRunner(
    ...
    ILogger<ShadowRunner> logger) : IShadowRunner
{
    internal const string RunDateKey = "runDate";
    internal const string SimulatedKey = "simulated";

    public async Task<ShadowRunOutcome> RunAsync(
        DateOnly runDate, bool simulated = false, CancellationToken cancellationToken = default)
    {
        ...
        DateOnly today = runDate;

        IReadOnlyList<StrategyTemplate> templates = await templateRepository.GetActiveAsync(ct);
        if (templates.Count == 0)
        {
            logger.LogInformation("Shadow run — no active templates.");
            return new ShadowRunOutcome(0, 0, 0, 0);
        }
        ...
        foreach (StrategyTemplate template in templates)
        {
            ...
            logger.LogInformation("Shadow run — created shadow portfolio for template {Template}.", template.Key);
        }
        ...
        IReadOnlyList<RankedTicker> ranked = await recommendationsApi.GetRankedTickersForDateAsync(runDate, simulated, ct);
        ...
        foreach (ShadowPortfolio portfolio in existing)
        {
            ...
            bool dueRebalance = !portfolio.IsInvested
                || ReviewSchedule.NextReview(RebalanceAnchor(portfolio), portfolio.RebalanceCadence,
                       runDate.ToDateTime(TimeOnly.MinValue).AddDays(-1)) <= runDate.ToDateTime(TimeOnly.MinValue);
        ...
        logger.LogInformation(
            "Shadow run [{RunDate}{Sim}] — valued {Valued}, rebalanced {Rebalanced}, skipped {Skipped} (missing prices), drawdown alerts {Alerts}.",
            runDate, simulated ? " SIM" : "", valued, rebalanced, skipped, drawdownAlerts);
        ...
        if (existing.Count > 0 && valued == 0)
        {
            ...
            logger.LogError(
                "Shadow run — wrote 0 snapshots for {Count} portfolio(s): {Reason}.",
        ...
        return new ShadowRunOutcome(valued, rebalanced, skipped, drawdownAlerts);
    }
    ...
    private bool LogDrawdown(ShadowPortfolio portfolio)
    {
        ...
        {
            logger.LogWarning(
                "Shadow run — {Template} crossed its drawdown alert: {Drawdown:P1} below high-water mark (threshold {Threshold:P0}).",
