...
/// profiled user when a daily run is <b>published</b> (§ 6.2) — never on mere
/// ingestion, so pending/quarantined runs stay invisible to users.
/// </summary>
public sealed class DailyRunPublishedIntegrationEventHandler(
    ISender sender,
    IPortfolioApi portfolioApi,
    IUsersApi usersApi,
    ILogger<DailyRunPublishedIntegrationEventHandler> logger)
    : IntegrationEventHandler<DailyRunPublishedIntegrationEvent>
{
    public override async Task Handle(DailyRunPublishedIntegrationEvent integrationEvent, CancellationToken cancellationToken = default)
    {
        IReadOnlyList<Guid> userIds = await portfolioApi.GetProfiledUserIdsAsync(cancellationToken);

        if (userIds.Count == 0)
        {
            logger.LogInformation("DailyRunPublished {DailyRunId}: no profiled users to notify.", integrationEvent.DailyRunId);
            return;
        }
        ...
        logger.LogInformation("DailyRunPublished {DailyRunId}: notified {Count}/{Total} users.",
